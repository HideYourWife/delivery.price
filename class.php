...
class DeliveryPrice extends CBitrixComponent
{
    ...
    {
        if (!empty($_SESSION['ZIP' . $this->arParams['ZIP_CODE']])) {
            $this->arResult = $_SESSION['ZIP' . $this->arParams['ZIP_CODE']];
            $this->includeComponentTemplate();
        }
        $request = Application::getInstance()->getContext()->getRequest();
        if (isset($this->arParams['ZIP_CODE'])) {
            $this->arResult["ZIP"] = $this->arParams['ZIP_CODE'];
        } else {
        ...
        try {
            if ($this->arResult["ZIP"]) {
                ...
                if ($location = Bitrix\Sale\Location\Admin\LocationHelper::getLocationsByZip($this->arResult["ZIP"], array('limit' => 1))->fetch()) {
                    ...
                    $this->arResult["ERRORS"][] = "Не удалось автоматически определить почтовый индекс.";
                }
        ...
        $_SESSION['ZIP'.$this->arResult["ZIP"]] = $this->arResult;
        $this->includeComponentTemplate();
    }
    ...
    function calculateDeliveries($order)
    {
        ...
        if (!empty($this->arDeliveryServiceAll)) {
            ...
            foreach ($this->arDeliveryServiceAll as $deliveryId => $deliveryObj) {
                ...
                $arDelivery["CODE"] = $deliveryObj->getCode();
